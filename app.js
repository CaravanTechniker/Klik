// UI v0.6 – categories + accordion + mobile tabs + info + feedback + share txt/pdf(print)
...
const els = {
  ...
  tabTrees: document.getElementById("tabTrees"),
...
// Demo data – nahradíme reálnymi stromami z content.json
const trees = [
  ...
  sk:{
    ...
    infoText:(contact)=>`
      <div style="line-height:1.55">
        <b>CaravanTechniker am Main</b><br>
        <div style="margin-top:10px">
          <b>E-mail:</b> ${escapeHtml(contact.email)}<br>
          <b>Mobil:</b> ${escapeHtml(contact.phone)}<br>
          <b>WhatsApp:</b> iba správa (na hovor neodpovedám kvôli vyťaženiu)<br>
        </div>
      </div>
    `,
...
// TODO: sem vložíš neskôr svoje reálne údaje
const CONTACT = {
  email: "aerophoto.de@gmail.com",
...
function applyLang() {
  ...
  els.infoBtn.textContent = "CaravanTechniker am Main"; // názov nech ostane konzistentný
...
function renderCats() {
  ...
  for (const [catName, items] of Object.entries(cats)) {
    ...
    head.addEventListener("click", () => {
      ...
      head.querySelector("div:last-child").textContent = cat.classList.contains("open") ? "▾" : "▸";
    });
    ...
    for (const tr of items) {
      ...
      row.addEventListener("click", () => {
        selectTree(tr.id);
        // na mobile automaticky prepnúť na diagnostiku
...
function wireInfo(){
  els.infoBtn.addEventListener("click", ()=>{
    els.infoContent.innerHTML = ui[lang].infoText(CONTACT);
...
function wireFeedback(){
  ...
  els.fbMail.addEventListener("click", ()=>{
    ...
    const subj = encodeURIComponent(lang==="sk" ? "Pripomienky k aplikácii Wohnmobil Diagnose" : "Feedback zur App Wohnmobil Diagnose");
...
function wireShare(){
  ...
  els.shareMenu.addEventListener("click", async (e)=>{
    ...
    if(b.dataset.share === "pdf"){
      // Bez knižníc: otvoríme print view, user uloží ako PDF
      openPrintView();
    }
  });
}
...
function wireButtons(){
  ...
  els.resetBtn.addEventListener("click", ()=>{
    ...
    toast(ui[lang].reset);
    renderCats();
    if (window.matchMedia("(max-width: 980px)").matches) setView("trees");
  });
}

function setView(v){
  els.wrap.dataset.view = v;
  els.tabTrees.classList.toggle("active", v==="trees");
  els.tabDiag.classList.toggle("active", v==="diag");
}

function wireTabs(){
  els.tabTrees.addEventListener("click", ()=> setView("trees"));
...
function init(){
  // default view mobile
  setView(window.matchMedia("(max-width: 980px)").matches ? "trees" : "trees");
  ...
  // Logo placeholder: neskôr sem dáš URL obrázka womo
